...
export default function ReportPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-200 pb-20 print:bg-white font-black text-slate-950 uppercase">
      ...
      <div className="bg-white max-w-[210mm] mx-auto p-[15mm] shadow-2xl min-h-[297mm] border-t-[20px] border-slate-950 print:max-w-none print:shadow-none print:m-0 print:border-none print:p-[4mm] font-black relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex justify-between border-b-8 border-slate-950 print:border-b-2 pb-10 print:pb-2 mb-10 print:mb-2 items-center font-black">
            <div className="space-y-4 print:space-y-1 font-black">
              <h1 className="text-4xl print:text-[13pt] tracking-tighter leading-none font-black text-slate-950">SHINING STARS - MA THWE</h1>
              <h2 className="text-slate-600 text-sm print:text-[8pt] tracking-[0.5em] font-black print:mt-0.5">PRIVATE HIGH SCHOOL</h2>
              <div className="pt-6 print:pt-1 flex flex-col print:flex-row print:gap-6 space-y-1 print:space-y-0 text-[10px] print:text-[7pt] text-slate-500 tracking-widest font-black uppercase">
                <p>REPORT DATE: {new Date(new Date().getTime() + (6*60+30)*60*1000).toISOString().split('T')[0]}</p>
                <p>RANGE: {startDate || 'START'} TO {endDate || 'PRESENT'}</p>
                <p className="print:hidden text-slate-950">STATUS: OFFICIAL EXPENDITURE LEDGER</p>
              </div>
            </div>
            <div className="relative w-32 h-32 print:w-12 print:h-12 font-black bg-white rounded-2xl p-2 print:p-0 border border-slate-200 print:border-0">
          ...
          {Object.keys(groupedData).length > 0 ? Object.keys(groupedData).map((cat) => (
            <div key={cat} className="mb-12 print:mb-2 last:mb-0 font-black">
              <div className="flex items-center justify-between bg-slate-100 text-slate-950 px-6 print:px-2 py-4 print:py-0.5 rounded-2xl print:rounded-none mb-4 print:mb-1 border-l-[10px] print:border-l-[3px] border-slate-400 font-black uppercase">
                <h3 className="text-lg print:text-[9pt] tracking-widest font-black text-slate-950">{cat}</h3>
                <FileSpreadsheet size={20} className="text-slate-400 print:hidden" />
              </div>
              <table className="w-full border-collapse font-black">
                <thead>
                  <tr className="bg-slate-50 border-b-2 border-slate-300 text-[10px] print:text-[7pt] text-slate-600 font-black uppercase">
                    <th className="py-4 print:py-0.5 px-3 print:px-1 text-left font-black w-32 print:w-24">DATE / ID</th>
                    <th className="py-4 print:py-0.5 px-3 print:px-1 text-left font-black">PARTICULARS & ACCOUNTS</th>
                    <th className="py-4 print:py-0.5 px-3 print:px-1 text-right font-black w-48 print:w-32">AMOUNT (MMK)</th>
                  </tr>
                </thead>
                <tbody className="text-[11px] print:text-[8pt] font-black uppercase">
                  {groupedData[cat].map((v: any, idx: number) => (
                    <tr key={idx} className="border-b print:border-b border-slate-200 print:border-slate-300 font-black hover:bg-slate-50 print:break-inside-avoid print:leading-tight">
                      <td className="py-4 print:py-[3px] px-3 print:px-1 align-top font-black">
                        <span className="text-[10px] print:text-[7pt] block mb-1 print:mb-0 font-black text-slate-500">{v.date}</span>
                        <span className="text-slate-950 text-xs print:text-[7.5pt] font-black">{v.voucherno}</span>
                      </td>
                      <td className="py-4 print:py-[3px] px-3 print:px-1 font-black">
                        <div className="text-sm print:text-[8pt] leading-tight mb-2 print:mb-0 font-black text-slate-950 flex items-center gap-2">
                           {v.item}
                           {v.type === 'Cash In' && <span className="text-[8px] bg-slate-200 text-slate-950 px-1 py-0.5 rounded">IN</span>}
                        </div>
                        <div className="text-[9px] print:text-[7pt] text-slate-500 font-black flex flex-wrap gap-1 items-center">
                        ...
                        {v.note && (
                          <div className="mt-2 print:mt-0 p-2 print:p-0 bg-slate-50 print:bg-transparent text-slate-700 text-[10px] print:text-[7pt] rounded-xl border-l-2 border-slate-300 print:border-0 font-black w-fit max-w-full print:italic">
                            📝 {v.note}
                          </div>
                        )}
                      </td>
                      <td className={`py-4 print:py-[3px] px-3 print:px-1 text-right text-lg print:text-[8.5pt] font-black align-top ${v.type === 'Cash In' ? 'text-emerald-600' : 'text-slate-950'}`}>
                ...
                <tfoot>
                  <tr className="bg-slate-100 text-slate-950 font-black border-t-4 border-slate-300">
                    <td colSpan={2} className="p-4 print:p-0.5 print:px-1 text-right text-xs print:text-[7pt] tracking-widest uppercase font-black text-slate-600">SUB-TOTAL ({cat})</td>
                    <td className="p-4 print:p-0.5 print:px-1 text-right text-xl print:text-[9pt] font-black">
          ...
          <div className="mt-16 print:mt-3 pt-8 print:pt-1 border-t-8 print:border-t-2 border-slate-950 bg-slate-50 print:bg-transparent p-10 print:p-1 flex justify-between items-center rounded-3xl print:rounded-none font-black break-inside-avoid">
            <span className="text-xl print:text-[9pt] tracking-[0.4em] text-slate-600 font-black uppercase">NET BALANCE LEDGER</span>
            <span className={`text-6xl print:text-[13pt] font-black ${grandTotal < 0 ? 'text-rose-600' : 'text-slate-950'}`}>
              {grandTotal < 0 ? '-' : '+'} {Math.abs(grandTotal).toLocaleString()} <span className="text-sm opacity-50 font-black text-slate-500">MMK</span>
            </span>
          </div>

          <div className="mt-32 print:mt-6 grid grid-cols-3 gap-20 print:gap-8 text-center text-[10px] print:text-[7pt] tracking-widest font-black uppercase break-inside-avoid text-slate-600">
            <div className="space-y-24 print:space-y-6 font-black"><div className="h-[3px] bg-slate-950 w-full"></div><span>ACCOUNTANT SIGNATURE</span></div>
            <div className="space-y-24 print:space-y-6 font-black"><div className="h-[3px] bg-slate-950 w-full"></div><span>OFFICE VERIFIED</span></div>
            <div className="space-y-24 print:space-y-6 font-black"><div className="h-[3px] bg-slate-950 w-full"></div><span>APPROVED BY</span></div>
          </div>
        </div>
      </div>
