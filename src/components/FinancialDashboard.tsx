...
export default function FinancialDashboard({ vouchers = [], onRefresh }: { vouchers: any[], onRefresh?: () => void }) {
  ...
  // Audit log — newest date first
  const groupedAuditLog = useMemo(()=>{
    const g: Record<string,Record<string,any[]>>={};
    // sort filtered newest first before grouping
        ...
        {Object.keys(groupedAuditLog).map(catName=>{
          ...
          return (
            <div key={catName} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              ...
              {isOpen&&Object.keys(groupedAuditLog[catName]).map(subName=>(
                <div key={subName}>
                      ...
                      <div key={idx} className="px-4 py-3 flex items-start gap-3">
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                            ...
                            {v.vendor&&<span className="text-[9px] text-slate-400 truncate max-w-[80px]">{v.vendor}</span>}
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            {v.account&&<span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[8px] font-black border border-slate-200">{v.account}</span>}
                            {v.entered_by&&<span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[8px] font-black border border-slate-200">{v.entered_by}</span>}
