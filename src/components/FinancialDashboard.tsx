...
function getPresetDates(preset: string): { startDate: string; endDate: string } {
  ...
  if (preset === '7d') {
    ...
    return { startDate: toStr(s), endDate: today };
  }
  if (preset === '30d') {
  ...
  if (preset === 'last') {
    ...
    return { startDate: toStr(s), endDate: toStr(d) };
  }
...
export default function FinancialDashboard({ vouchers = [], onRefresh }: { vouchers: any[], onRefresh?: () => void }) {
  ...
  const subCategoryOptions = useMemo(() => {
    ...
    normalizedData.filter(v=>!filter.category||v.category===filter.category).forEach(v=>{
      [v.sub1,v.sub2,v.sub3,v.sub4,v.sub5].filter(Boolean).forEach(x=>s.add(x));
    });
  ...
  const groupedAuditLog = useMemo(() => {
    ...
    filtered.forEach(v => {
      ...
      if (!g[v.category][v.sub1]) g[v.category][v.sub1]=[];
      g[v.category][v.sub1].push(v);
    });
    return g;
  }, [filtered]);
  ...
  const PRESETS = [
    { key:'7d',    label:'7 ရက်'   },
    { key:'30d',   label:'30 ရက်'  },
    { key:'month', label:'ဒီလ'     },
    { key:'last',  label:'ပြီးခဲ့တဲ့လ' },
  ];

  const FSelect = ({ label, value, options, onChange }: any) => (
    <div className="relative">
      <select className="appearance-none w-full pl-3 pr-7 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[11px] outline-none font-black text-slate-950 uppercase cursor-pointer"
    ...
    <div className="space-y-5 font-black text-slate-950 uppercase">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3 print:hidden">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={()=>handleRefresh()} disabled={isRefreshing}
            className="ml-auto p-2 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 disabled:opacity-50">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-200">
          ...
          <p className="text-lg font-black text-emerald-700 leading-tight">{fmt(analytics.totalIn)}</p>
        ...
        <div className="bg-rose-50 p-4 rounded-2xl border border-rose-200">
          ...
          <p className="text-lg font-black text-rose-700 leading-tight">{fmt(analytics.totalOut)}</p>
          <p className="text-[9px] text-slate-400">MMK</p>
        </div>
        <div className={`p-4 rounded-2xl border ${analytics.balance>=0?'bg-purple-50 border-purple-200':'bg-rose-100 border-rose-300'}`}>
          ...
          <p className={`text-lg font-black leading-tight ${analytics.balance>=0?'text-purple-700':'text-rose-700'}`}>{fmt(analytics.balance)}</p>
          <p className="text-[9px] text-slate-400">MMK</p>
        </div>
      </div>
      ...
      {analytics.dailyTrends.length > 0 && (
        <div className="bg-white p-4 rounded-2xl border border-slate-200">
          <h3 className="text-[10px] text-slate-500 mb-4 flex items-center gap-2 tracking-widest"><TrendingUp size={13}/> DAILY TREND</h3>
          <div className="h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.dailyTrends} barGap={2} margin={{left:-20,right:4,top:4,bottom:0}}>
                ...
                <YAxis tick={{fontSize:7,fontWeight:900,fill:'#94a3b8'}} tickLine={false} axisLine={false} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}K`:v}/>
      ...
      {analytics.monthlyTrends.length > 0 && (
        <div className="bg-white p-4 rounded-2xl border border-slate-200">
          <h3 className="text-[10px] text-slate-500 mb-4 flex items-center gap-2 tracking-widest"><TrendingUp size={13}/> MONTHLY TREND</h3>
          <div className="h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.monthlyTrends} barGap={2} margin={{left:-20,right:4,top:4,bottom:0}}>
                <CartesianGrid vertical={false} stroke="#f1f5f9"/>
                <XAxis dataKey="month" tick={{fontSize:8,fontWeight:900,fill:'#94a3b8'}} tickLine={false} axisLine={false} tickFormatter={m=>m.slice(5)+' လ'}/>
                <YAxis tick={{fontSize:7,fontWeight:900,fill:'#94a3b8'}} tickLine={false} axisLine={false} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}K`:v}/>
                <Tooltip formatter={(v:any)=>fmt(Number(v))+' MMK'} labelStyle={{fontSize:10,fontWeight:900}} labelFormatter={m=>m+' လ'}/>
                <Bar dataKey="income"  fill="#10b981" name="IN"  radius={[3,3,0,0]}/>
                <Bar dataKey="expense" fill="#f43f5e" name="OUT" radius={[3,3,0,0]}/>
              </BarChart>
            </ResponsiveContainer>
      ...
      {/* ── EXPENSE PIE (no legend, tooltip only) ── */}
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200">
          ...
          <div className="flex gap-3 items-start">
            ...
            <div className="w-[130px] h-[130px] shrink-0">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={analytics.categories} innerRadius={38} outerRadius={58} paddingAngle={3} dataKey="value" stroke="none">
      ...
      {/* ── AUDIT LOG — card style ── */}
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-1"><ListChecks className="text-slate-400" size={16}/><h2 className="text-sm tracking-widest font-black">DETAILED AUDIT LOG</h2></div>
        {Object.keys(groupedAuditLog).map(catName => {
          const allRows  = Object.values(groupedAuditLog[catName]).flat() as any[];
          const catOut   = allRows.filter((v:any)=>v.type!=='Cash In').reduce((s:number,v:any)=>s+v.cost_total,0);
          const catIn    = allRows.filter((v:any)=>v.type==='Cash In').reduce((s:number,v:any)=>s+v.cost_total,0);
          return (
              ...
              {Object.keys(groupedAuditLog[catName]).map(subName => (
                <div key={subName}>
                  <div className="px-4 py-2 bg-slate-50 border-b border-slate-100">
                    <p className="text-[10px] text-slate-400 font-black border-l-2 border-slate-300 pl-2">{subName}</p>
                  </div>
                  <div className="divide-y divide-slate-50">
                    {groupedAuditLog[catName][subName].map((v:any,idx:number) => (
                      <div key={idx} className="px-4 py-3 flex items-start gap-3">
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                            ...
                            <span className="text-[9px] text-slate-400">{v.voucherno}</span>
                            {v.vendor && <span className="text-[9px] text-slate-400 truncate">{v.vendor}</span>}
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[8px] font-black border border-slate-200">{v.account}</span>
                            <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[8px] font-black border border-slate-200">{v.entered_by}</span>
                            {[v.sub2,v.sub3,v.sub4,v.sub5].filter(Boolean).map((s:string,si:number)=>(
                              <span key={si} className="text-[8px] text-slate-400">› {s}</span>
                            ))}
                          </div>
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          ...
                          <div className="flex gap-1.5">
                            <button onClick={()=>handleDelete(v.voucherno)} className="p-1.5 bg-rose-50 text-rose-500 border border-rose-200 rounded-lg hover:bg-rose-600 hover:text-white transition-all print:hidden">
                              <Trash2 size={12}/>
                            </button>
                            {v.image_data
                              ? <button onClick={()=>setSelectedImg(v.image_data)} className="p-1.5 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-950 hover:text-white transition-all text-slate-600 print:hidden"><ImageIcon size={12}/></button>
                              : null}
                          </div>
                        </div>
                      </div>
