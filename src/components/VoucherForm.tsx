...
export default function VoucherForm({ onRefresh }: { onRefresh: () => void }) {
  ...
  const [addSubLevel, setAddSubLevel] = useState<number>(0);
  ...
  const getLastSerial = (prefix: string): number => {
    ...
    for (const [key, val] of Object.entries(config.lastSerials)) {
      if (key.trim().toUpperCase() === upperPrefix) return Number(val) || 0;
    }
    return 0;
  };
  ...
  // Auto‑generate when category changes (batch empty)
  useEffect(() => {
    if (itemList.length === 0 && category) {
      const newId = generateVrID(category, []);
      setVoucherno(newId);
    }
  ...
  const addItem = async () => {
    ...
    const newItem = {
      ...
      item_description: currentItem.brand ? `${currentItem.item_description} (${currentItem.brand})` : currentItem.item_description,
    ...
    <div className="relative grid grid-cols-1 lg:grid-cols-12 gap-0 font-black text-slate-950">
      ...
      {cropMode && (
        <div className="fixed inset-0 z-[300] bg-black flex flex-col select-none">
          <div className="bg-black px-4 py-3 flex items-center justify-between border-b border-white/10">
            <div><p className="text-white text-xs font-black tracking-widest uppercase">Manual Crop</p><p className="text-slate-400 text-[9px] mt-0.5">Corner & Edge ကိုဆွဲ၍ ဖြတ်ပါ</p></div>
            <div className="flex items-center gap-2">
              <button onClick={()=>{setCropMode(false); setRawSrc('');}} className="text-slate-400 text-[10px] font-black px-3 py-1.5 border border-white/20 rounded-lg">CANCEL</button>
              <button onClick={applyCrop} className="bg-emerald-500 text-white text-[10px] font-black px-4 py-1.5 rounded-lg flex items-center gap-1.5 shadow-lg active:scale-95 transition-transform"><Check size={12}/> CROP & ENHANCE</button>
            </div>
          </div>
          <div className="flex-1 overflow-hidden flex items-center justify-center p-3" onPointerMove={onCropPointerMove} onPointerUp={onCropPointerUp} onPointerCancel={onCropPointerUp}>
            <div className="relative inline-block" style={{maxWidth:'100%',maxHeight:'100%'}}>
              <img ref={cropImgRef} src={rawSrc} alt="crop" draggable={false} style={{display:'block',maxWidth:'calc(100vw - 24px)',maxHeight:'calc(100vh - 130px)',userSelect:'none'}}/>
              <div className="absolute inset-0 pointer-events-none" style={{background:`linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6))`, clipPath:`polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%, 0% 0%, ${cropRect.x*100}% ${cropRect.y*100}%, ${cropRect.x*100}% ${(cropRect.y+cropRect.h)*100}%, ${(cropRect.x+cropRect.w)*100}% ${(cropRect.y+cropRect.h)*100}%, ${(cropRect.x+cropRect.w)*100}% ${cropRect.y*100}%, ${cropRect.x*100}% ${cropRect.y*100}%)`}}/>
              <div className="absolute pointer-events-none" style={{left:`${cropRect.x*100}%`, top:`${cropRect.y*100}%`, width:`${cropRect.w*100}%`, height:`${cropRect.h*100}%`, border:'2px solid #34d399', boxSizing:'border-box'}}>
                {[1/3,2/3].map(p=>(
              ...
              <div onPointerDown={e=>onCropPointerDown(e,'move')} className="absolute cursor-move" style={{left:`${cropRect.x*100}%`,top:`${cropRect.y*100}%`,width:`${cropRect.w*100}%`,height:`${cropRect.h*100}%`}}/>
              {[
                ...
                <div key={h} onPointerDown={e=>onCropPointerDown(e,h)} style={{position:'absolute', cursor, left:`calc(${cx*100}% - 14px)`, top:`calc(${cy*100}% - 14px)`, width:28, height:28, zIndex:10, display:'flex', alignItems:'center', justifyContent:'center'}}>
                  <div style={{width: h.length===2?14:10, height: h.length===2?14:10, background:'#34d399', border:'2px solid white', borderRadius: h.length===2?3:2, boxShadow:'0 2px 8px rgba(0,0,0,0.5)'}}/>
                </div>
              ))}
            </div>
          </div>
      ...
      {cameraActive && (
        <div className="fixed inset-0 z-[300] bg-black flex flex-col">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="relative" style={{width:'78%',height:'58%'}}>
                {(['tl','tr','bl','br'] as const).map(pos=>(
                  <span key={pos} className="absolute w-8 h-8 block" style={{top:pos.startsWith('t')?0:'auto', bottom:pos.startsWith('b')?0:'auto', left:pos.endsWith('l')?0:'auto', right:pos.endsWith('r')?0:'auto', borderTop:pos.startsWith('t')?'3px solid #34d399':undefined, borderBottom:pos.startsWith('b')?'3px solid #34d399':undefined, borderLeft:pos.endsWith('l')?'3px solid #34d399':undefined, borderRight:pos.endsWith('r')?'3px solid #34d399':undefined}}/>
                ))}
              </div>
            </div>
          </div>
          <div className="bg-black py-8 flex items-center justify-around">
            ...
            <button onClick={captureFrame} className="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-2xl active:scale-95 transition-transform"><div className="w-16 h-16 rounded-full bg-white border-4 border-slate-300 flex items-center justify-center"><Camera size={28} className="text-slate-700"/></div></button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-200 font-black">
          <div className="space-y-2 font-black" ref={supplierRef}>
            ...
            <div className="relative">
              <input className="w-full bg-white border border-slate-300 p-3 pr-10 rounded-xl outline-none focus:border-slate-500 text-sm font-black text-slate-950" value={supplierSearch} onChange={e => { setSupplierSearch(e.target.value); setVendor(e.target.value); setSelectedSupplier(null); setSupplierDropdown(true); }} onFocus={() => setSupplierDropdown(true)} placeholder="SEARCH..."/>
              <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/>
              {supplierDropdown && (
                <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                  {filteredSuppliers.length > 0 ? filteredSuppliers.map((s: any, i: number) => (
                    <div key={i} className="px-4 py-2.5 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-0" onMouseDown={() => { setVendor(s.name); setSupplierSearch(s.name); setSelectedSupplier(s); setSupplierDropdown(false); }}>
                    ...
                    <div className="px-4 py-3 text-center space-y-2">
                      ...
                      <button onMouseDown={() => { setNewSupplier({ ...newSupplier, name: supplierSearch }); setShowNewSupplierForm(true); setSupplierDropdown(false); }} className="text-[10px] bg-slate-950 text-white px-3 py-1.5 rounded-lg font-black">+ ADD NEW SUPPLIER</button>
                    </div>
                  )}
                </div>
              )}
            ...
            {selectedSupplier && (
              <div className="space-y-1.5 pt-1">
                <div className="flex justify-between items-center mb-1"><span className="text-[10px] text-slate-400 tracking-widest">INFO (AUTO-FILLED)</span><div className="flex items-center gap-2"><button onClick={() => openEditModal(selectedSupplier)} className="text-[10px] text-slate-500 hover:text-slate-950 flex items-center gap-1 border border-slate-300 bg-white px-2 py-1 rounded-lg font-black">✏️ EDIT</button><button onClick={() => { setSelectedSupplier(null); setVendor(''); setSupplierSearch(''); }} className="text-[10px] text-slate-400 hover:text-slate-700 flex items-center gap-1"><X size={10}/> CLEAR</button></div></div>
                {[
                  ...
                  <div key={idx} className="flex items-center gap-2 bg-slate-100 border border-slate-200 rounded-xl px-3 py-2"><span className="text-slate-400 flex-shrink-0">{icon}</span><span className={`text-[11px] font-black flex-1 uppercase truncate ${value ? 'text-slate-950' : 'text-slate-300'}`}>{value || placeholder}</span></div>
                ))}
              </div>
            )}
            {showNewSupplierForm && (
          ...
          <div className="space-y-4 font-black">
            <div className="space-y-1">
              ...
              {showAddCat ? (
                <div className="flex gap-1">
                  <input autoFocus className="flex-1 p-2 bg-white border border-slate-400 rounded-xl text-xs font-black uppercase outline-none text-slate-950 placeholder:text-slate-300" placeholder="NEW CATEGORY" value={newCatInput} onChange={e => setNewCatInput(e.target.value)} onKeyDown={e => { if (e.key==='Enter') handleInlineAddCat(); if (e.key==='Escape') setShowAddCat(false); }}/>
                  <button type="button" onClick={handleInlineAddCat} disabled={catBusy || !newCatInput.trim()} className="px-3 bg-slate-950 text-white rounded-xl text-[10px] font-black disabled:opacity-40">{catBusy ? '…' : 'SAVE'}</button>
                  <button type="button" onClick={() => setShowAddCat(false)} className="px-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black">✕</button>
                </div>
              ) : (
                <select className="w-full p-3 bg-white border border-slate-300 rounded-xl text-xs uppercase font-black text-slate-950 focus:border-slate-500" value={category} onChange={e => { setCategory(e.target.value); setSub1(''); setSub2(''); setSub3(''); setSub4(''); setSub5(''); setAddSubLevel(0); }}>
            ...
            {([
              ...
              <div key={level} className="space-y-1">
                <div className="flex items-center justify-between"><label className="text-[10px] text-slate-500 uppercase font-black">{label}</label>{addSubLevel !== level && (<button type="button" onClick={() => { setAddSubLevel(level); setNewSubInput(''); }} className="text-[9px] text-slate-400 hover:text-slate-950 border border-dashed border-slate-300 px-2 py-0.5 rounded-lg font-black uppercase hover:border-slate-500 transition-all">+ ADD</button>)}</div>
                {addSubLevel === level ? (
                  <div className="flex gap-1">
                    <input autoFocus className="flex-1 p-2 bg-white border border-slate-400 rounded-xl text-xs font-black uppercase outline-none text-slate-950 placeholder:text-slate-300" placeholder={`NEW ${label}`} value={newSubInput} onChange={e => setNewSubInput(e.target.value)} onKeyDown={e => { if (e.key==='Enter') handleInlineAddSub(); if (e.key==='Escape') setAddSubLevel(0); }}/>
                    <button type="button" onClick={handleInlineAddSub} disabled={catBusy || !newSubInput.trim()} className="px-3 bg-slate-950 text-white rounded-xl text-[10px] font-black disabled:opacity-40">{catBusy ? '…' : 'SAVE'}</button>
                    <button type="button" onClick={() => setAddSubLevel(0)} className="px-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black">✕</button>
                  </div>
                ) : opts.length > 0 ? (
                  <select className="w-full p-3 bg-white border border-slate-300 rounded-xl text-xs uppercase font-black text-slate-950 focus:border-slate-500" value={val} onChange={e => setter(e.target.value)}>
          ...
          <div className="space-y-4 font-black">
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center font-black"><Hash size={12} className="mr-1"/> QTY</label><input type="number" step="any" className="w-full p-4 bg-white border border-slate-300 rounded-2xl text-xl text-center text-slate-950 outline-none focus:border-slate-500 font-black" value={currentItem.count} onChange={e => { const qty = e.target.value; const rate = parseFloat(currentItem.cost_piece); const total = parseFloat(currentItem.cost_total); let next = { ...currentItem, count: qty }; const q = parseFloat(qty); if (!isNaN(q) && q > 0) { if (!isNaN(rate) && rate > 0) next.cost_total = String(Math.round(q * rate)); else if (!isNaN(total) && total > 0) next.cost_piece = String(Math.round(total / q)); } setCurrentItem(next); }}/></div>
                <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center font-black"><Banknote size={12} className="mr-1"/> RATE</label><input type="number" className="w-full p-4 bg-white border border-slate-300 rounded-2xl text-xl text-center text-slate-950 outline-none focus:border-slate-500 font-black" value={currentItem.cost_piece} onChange={e => { const rate = e.target.value; const qty = parseFloat(currentItem.count); let next = { ...currentItem, cost_piece: rate }; const r = parseFloat(rate); if (!isNaN(r) && r > 0 && !isNaN(qty) && qty > 0) next.cost_total = String(Math.round(qty * r)); setCurrentItem(next); }}/></div>
              </div>
              <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center font-black">💵 TOTAL</label><input type="number" className="w-full p-4 bg-slate-950 text-white border-0 rounded-2xl text-2xl text-center outline-none font-black placeholder:text-slate-600" placeholder="0" value={currentItem.cost_total} onChange={e => { const total = e.target.value; const qty = parseFloat(currentItem.count); const rate = parseFloat(currentItem.cost_piece); let next = { ...currentItem, cost_total: total }; const t = parseFloat(total); if (!isNaN(t) && t > 0) { if (!isNaN(qty) && qty > 0) next.cost_piece = String(Math.round(t / qty)); else if (!isNaN(rate) && rate > 0) next.count = String(Math.round(t / rate)); } setCurrentItem(next); }}/></div>
            </div>
            <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center font-black"><MessageSquare size={12} className="mr-2"/> NOTES</label><textarea className="w-full p-4 bg-white border border-slate-300 rounded-xl text-[10px] outline-none focus:border-slate-500 h-16 resize-none font-black text-slate-950" value={currentItem.note} onChange={e => setCurrentItem({ ...currentItem, note: e.target.value })}/></div>
            {[category, sub1, sub2, sub3].some(s => /ferry|bus\s*\d*|ယာဉ်/i.test(s)) && (
            <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center font-black"><span className="mr-1">🛣</span> KM / ODOMETER <span className="text-slate-300 normal-case font-normal ml-1">(optional)</span></label><input type="number" className="w-full p-4 bg-white border border-slate-300 rounded-2xl text-xl text-center text-slate-950 outline-none focus:border-slate-500 font-black" placeholder="—" value={currentItem.km} onChange={e => setCurrentItem({ ...currentItem, km: e.target.value })}/></div>
            )}
            <div className="space-y-1"><label className="text-[10px] text-slate-500 uppercase flex items-center gap-1.5 font-black"><ScanLine size={12}/> SCAN / PHOTO <span className="text-emerald-600 bg-emerald-50 border border-emerald-200 text-[8px] px-1.5 py-0.5 rounded-full font-black ml-1">AUTO CROP + ENHANCE</span><span className="text-slate-300 normal-case font-normal text-[9px]">(optional)</span></label><div className="relative min-h-28 bg-white rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden">
                {scanProcessing && (<div className="absolute inset-0 bg-slate-950 flex flex-col items-center justify-center gap-3 z-10"><div className="relative"><Zap size={32} className="text-emerald-400 animate-pulse"/><div className="absolute inset-0 rounded-full bg-emerald-400/20 animate-ping"/></div><p className="text-white text-[10px] font-black tracking-widest animate-pulse">{scanProgress}</p><div className="w-32 h-1 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-emerald-400 rounded-full animate-pulse" style={{width:'60%'}}/></div></div>)}
                {!scanProcessing && image && (<><img src={image} className="w-full max-h-52 object-contain"/><div className="absolute top-2 left-2 bg-emerald-500 text-white text-[9px] font-black px-2 py-1 rounded-full flex items-center gap-1 shadow"><Check size={9}/> SCANNED & ENHANCED</div><div className="absolute top-2 right-2 flex gap-1.5">{rawSrc && (<button onClick={()=>setCropMode(true)} title="Crop ပြန်လုပ်" className="bg-violet-100 text-violet-600 border border-violet-200 p-1.5 rounded-full shadow-sm"><ScanLine size={12}/></button>)}<button onClick={() => { setImage(''); openCamera(); }} title="Camera ထပ်ရိုက်" className="bg-blue-100 text-blue-600 border border-blue-200 p-1.5 rounded-full shadow-sm"><Camera size={12}/></button><button onClick={() => scanFileRef.current?.click()} title="Gallery မှရွေး" className="bg-amber-100 text-amber-600 border border-amber-200 p-1.5 rounded-full shadow-sm"><RotateCcw size={12}/></button><button onClick={() => { setImage(''); setRawSrc(''); }} title="ဖျက်" className="bg-rose-100 text-rose-600 border border-rose-200 p-1.5 rounded-full shadow-sm"><Trash2 size={12}/></button></div></>)}
                {!scanProcessing && !image && (<div className="flex flex-col items-center gap-3 py-5"><div className="flex gap-3"><button onClick={openCamera} className="flex flex-col items-center gap-1.5 bg-slate-950 text-white px-6 py-3 rounded-xl hover:bg-slate-800 active:scale-95 transition-all shadow-md"><Camera size={22}/><span className="text-[9px] font-black uppercase tracking-wider">Camera</span></button><button onClick={() => scanFileRef.current?.click()} className="flex flex-col items-center gap-1.5 bg-slate-100 text-slate-700 border border-slate-200 px-6 py-3 rounded-xl hover:bg-slate-200 active:scale-95 transition-all"><Upload size={22}/><span className="text-[9px] font-black uppercase tracking-wider">Gallery</span></button></div><div className="flex items-center gap-1.5 text-slate-400 text-[9px]"><ScanLine size={10}/><span>ရိုက်ပြီးတာနဲ့ Auto Crop + Enhance လုပ်မည်</span></div></div>)}
                <input ref={scanFileRef} type="file" accept="image/*" className="hidden" onChange={handleScanFile}/>
              </div></div>
            <button onClick={addItem} disabled={imageUploading} className={`w-full py-5 rounded-[1.5rem] text-sm transition-all flex items-center justify-center uppercase font-black border shadow-sm ${imageUploading ? 'bg-amber-50 border-amber-200 text-amber-700 cursor-wait' : 'bg-slate-200 border-slate-300 text-slate-950 hover:bg-slate-300'}`}>{imageUploading ? <><RefreshCcw className="mr-2 animate-spin" size={16}/> UPLOADING PHOTO...</> : <><Plus className="mr-2" size={20} strokeWidth={4}/> ADD TO BATCH</>}</button>
          </div>
        </div>
      </div>
      <div className="lg:col-span-4 flex flex-col bg-slate-50 border-l border-slate-200 font-black">
        ...
        <div className="flex-grow p-5 space-y-4 overflow-y-auto max-h-[500px] font-black">
          {itemList.map(i => (
            <div key={i.id} className={`bg-white rounded-2xl shadow-sm border border-slate-200 border-l-[6px] ${i.type === 'Cash In' ? 'border-l-emerald-400' : 'border-l-rose-400'} font-black overflow-hidden`}>
              <div className="p-4"><div className="flex justify-between items-start"><div className="space-y-1 flex-1 min-w-0 mr-2"><p className="text-[10px] text-slate-500">{i.voucherno}</p><p className="text-xs leading-tight font-black text-slate-950 truncate">{i.item_description}</p><p className="text-[8px] text-slate-600 uppercase">[{i.entered_by} • {i.account}]</p></div><div className="flex items-center gap-1 shrink-0"><button onClick={() => { setEditingItemId(i.id); setEditItem({ ...i }); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button onClick={() => setItemList(itemList.filter(x => x.id !== i.id))} className="p-1.5 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all"><Trash2 size={13}/></button></div></div>{i.note && <p className="text-[9px] text-slate-500 mt-2">NOTE: {i.note}</p>}{i.km > 0 && <p className="text-[9px] text-blue-600 mt-1">🛣 {i.km.toLocaleString()} km</p>}{i.image_data && <p className="text-[9px] text-emerald-600 mt-1">📷 PHOTO ATTACHED</p>}<div className="flex justify-between items-end mt-3"><p className="text-[9px] text-slate-500">{i.count} × {Number(i.cost_piece).toLocaleString()} MMK</p><p className="text-sm font-black">{Number(i.cost_total).toLocaleString()}</p></div></div>
              {editingItemId === i.id && editItem && (
                <div className="border-t-2 border-blue-200 bg-blue-50 p-4 space-y-3">
                  <p className="text-[9px] text-blue-600 uppercase tracking-widest font-black flex items-center gap-1"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> Voucher ပြင်ဆင်ရန်</p>
                  <div className="space-y-1"><label className="text-[9px] text-blue-500 uppercase font-black">Item Description</label><input className="w-full p-2.5 bg-white border border-blue-200 rounded-xl text-xs font-black outline-none focus:border-blue-500 text-slate-950" value={editItem.item_description} onChange={e => setEditItem((prev: any) => ({ ...prev, item_description: e.target.value }))}/></div>
                  <div className="grid grid-cols-2 gap-2"><div className="space-y-1"><label className="text-[9px] text-blue-500 uppercase font-black">QTY</label><input type="number" step="any" className="w-full p-2.5 bg-white border border-blue-200 rounded-xl text-xs font-black outline-none focus:border-blue-500 text-center text-slate-950" value={editItem.count} onChange={e => { const qty = parseFloat(e.target.value) || 0; const rate = parseFloat(editItem.cost_piece) || 0; setEditItem((prev: any) => ({ ...prev, count: e.target.value, cost_total: rate > 0 ? Math.round(qty * rate) : prev.cost_total })); }}/></div><div className="space-y-1"><label className="text-[9px] text-blue-500 uppercase font-black">Rate</label><input type="number" className="w-full p-2.5 bg-white border border-blue-200 rounded-xl text-xs font-black outline-none focus:border-blue-500 text-center text-slate-950" value={editItem.cost_piece} onChange={e => { const rate = parseFloat(e.target.value) || 0; const qty = parseFloat(editItem.count) || 0; setEditItem((prev: any) => ({ ...prev, cost_piece: e.target.value, cost_total: qty > 0 ? Math.round(qty * rate) : prev.cost_total })); }}/></div></div>
                  <div className="space-y-1"><label className="text-[9px] text-blue-500 uppercase font-black">Total (MMK)</label><input type="number" className="w-full p-3 bg-slate-950 text-white border-0 rounded-xl text-lg text-center font-black outline-none" value={editItem.cost_total} onChange={e => setEditItem((prev: any) => ({ ...prev, cost_total: parseFloat(e.target.value) || 0 }))}/></div>
                  <div className="space-y-1"><label className="text-[9px] text-blue-500 uppercase font-black">Note</label><input className="w-full p-2.5 bg-white border border-blue-200 rounded-xl text-xs font-black outline-none focus:border-blue-500 text-slate-950" value={editItem.note || ''} onChange={e => setEditItem((prev: any) => ({ ...prev, note: e.target.value }))} placeholder="Note..."/></div>
                  <div className="flex gap-2 pt-1"><button onClick={() => { setItemList(itemList.map(x => x.id === editingItemId ? { ...editItem } : x)); setEditingItemId(null); setEditItem(null); }} className="flex-1 bg-blue-600 text-white py-2.5 rounded-xl text-[11px] font-black flex items-center justify-center gap-1.5 hover:bg-blue-700 transition-colors"><Save size={12}/> SAVE EDIT</button><button onClick={() => { setEditingItemId(null); setEditItem(null); }} className="px-4 bg-white text-slate-600 border border-slate-200 rounded-xl text-[11px] font-black hover:bg-slate-100 transition-colors">CANCEL</button></div>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="bg-slate-200 p-8 space-y-6 font-black border-t border-slate-300">
          <div className="flex justify-between items-end border-b border-slate-400 pb-4"><span className="text-slate-600 text-[10px] tracking-widest">TOTAL</span><span className="text-slate-950 text-4xl font-black">{itemList.reduce((s, x) => s + x.cost_total, 0).toLocaleString()}</span></div>
          <button onClick={handleFinalSubmit} disabled={submitStatus === 'processing' || itemList.length === 0} className={`w-full py-5 rounded-2xl text-xs transition-all flex items-center justify-center font-black ${submitStatus === 'success' ? 'bg-emerald-600 text-white' : submitStatus === 'idle' ? 'bg-slate-950 text-white shadow-md hover:bg-slate-800' : submitStatus === 'processing' ? 'bg-slate-300 text-slate-950' : 'bg-rose-100 text-rose-900 border border-rose-300'}`}>
      ...
      {showEditModal && editingSupplier && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            <div className="bg-slate-950 text-white px-6 py-4 flex justify-between items-center"><div><p className="text-[10px] text-slate-400 tracking-widest">EDIT SUPPLIER</p><p className="text-sm font-black uppercase">{editingSupplier.name}</p></div><button onClick={() => setShowEditModal(false)} className="text-slate-400 hover:text-white"><X size={18}/></button></div>
            <div className="p-6 space-y-4">
              <div className="space-y-1"><label className="text-[10px] text-slate-400 tracking-widest flex items-center gap-1"><Phone size={10}/> PHONE 1</label><input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-slate-500 font-black text-slate-950" value={editForm.phone1} onChange={e => setEditForm({ ...editForm, phone1: e.target.value })} placeholder="09..."/></div>
              <div className="space-y-1"><label className="text-[10px] text-slate-400 tracking-widest flex items-center gap-1"><Phone size={10}/> PHONE 2</label><input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-slate-500 font-black text-slate-950" value={editForm.phone2} onChange={e => setEditForm({ ...editForm, phone2: e.target.value })} placeholder="09..."/></div>
              <div className="space-y-1"><label className="text-[10px] text-slate-400 tracking-widest flex items-center gap-1"><Phone size={10}/> PHONE 3</label><input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-slate-500 font-black text-slate-950" value={editForm.phone3} onChange={e => setEditForm({ ...editForm, phone3: e.target.value })} placeholder="09..."/></div>
              <div className="space-y-1"><label className="text-[10px] text-slate-400 tracking-widest flex items-center gap-1"><MapPin size={10}/> ADDRESS</label><input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-slate-500 font-black text-slate-950" value={editForm.address} onChange={e => setEditForm({ ...editForm, address: e.target.value })} placeholder="ADDRESS..."/></div>
              <div className="space-y-2"><label className="text-[10px] text-slate-400 tracking-widest flex items-center gap-1"><Briefcase size={10}/> SERVICES</label><div className="flex flex-wrap gap-2 min-h-[32px]">{editServices.map((svc, idx) => (<span key={idx} className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 text-slate-950 text-[11px] font-black px-3 py-1.5 rounded-full uppercase">{svc}<button onClick={() => setEditServices(editServices.filter((_, i) => i !== idx))} className="text-slate-400 hover:text-rose-500"><X size={10}/></button></span>))}{editServices.length === 0 && <span className="text-[10px] text-slate-300 italic">NO SERVICES YET</span>}</div><div className="flex gap-2"><input className="flex-1 bg-slate-50 border border-slate-200 p-2.5 rounded-xl text-[11px] outline-none focus:border-slate-500 font-black text-slate-950" placeholder="ADD SERVICE..." value={newServiceInput} onChange={e => setNewServiceInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && newServiceInput.trim()) { e.preventDefault(); if (!editServices.includes(newServiceInput.trim())) setEditServices([...editServices, newServiceInput.trim()]); setNewServiceInput(''); } }}/><button onClick={() => { if (!newServiceInput.trim()) return; if (!editServices.includes(newServiceInput.trim())) setEditServices([...editServices, newServiceInput.trim()]); setNewServiceInput(''); }} className="bg-slate-200 hover:bg-slate-300 text-slate-950 px-3 rounded-xl font-black"><Plus size={14}/></button></div><p className="text-[9px] text-slate-300">ENTER နှိပ်ရင်လည်း ထည့်နိုင်သည်</p></div>
            </div>
            <div className="px-6 pb-6 flex gap-3"><button onClick={saveEditedSupplier} disabled={editSaving} className="flex-1 bg-slate-950 text-white py-3 rounded-xl text-xs font-black flex items-center justify-center gap-2 disabled:opacity-50">{editSaving ? <><RefreshCcw size={14} className="animate-spin"/> SAVING...</> : <><Save size={14}/> SAVE CHANGES</>}</button><button onClick={() => setShowEditModal(false)} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-xl text-xs font-black">CANCEL</button></div>
          </div>
        </div>
      )}
      {/* Debug Panel */}
